...
(* Derivation with no unfinished leaves - proof *)
Definition pf {X : Type} rules concl := @derrec X rules (fun _ => False) concl.
Ltac unf_pf := unfold pf.
Ltac unf_pf_all := unfold pf in *.

Definition pfs {X : Type} rules concls := @dersrec X rules (fun _ => False) concls.
Ltac unf_pfs := unfold pfs.
Ltac unf_pfs_all := unfold pfs in *.
