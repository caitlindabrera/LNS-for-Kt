...
Inductive merge {V : Set} :  (@LNS V) ->  (@LNS V) ->  (@LNS V) -> Type :=
| merge_nilL ns1 ns2 ns3 : ns1 = [] -> ns3 = ns2 -> merge ns1 ns2 ns3
| merge_nilR ns1 ns2 ns3 : ns2 = [] -> ns3 = ns1 -> merge ns1 ns2 ns3
| merge_step Γ Δ Σ Π d ns1 ns2 ns3 ns4 ns5 ns6 s1 s2 s3 :
    s1 = (Γ,Δ,d) -> s2 = (Σ,Π,d) -> s3 = (Γ++Σ,Δ++Π,d) ->
    merge ns1 ns2 ns3 ->
    ns4 = s1 :: ns1 -> ns5 = s2 :: ns2 -> ns6 = s3 :: ns3 ->
    merge ns4 ns5 ns6.
...
Lemma merge_app_struct_equiv_strR : forall {V : Set} G H1 s GHs,
    merge G (H1 ++ [s]) GHs ->
    struct_equiv_str G (H1 ++ [s]) ->
...
Ltac get_hyp_merge_weakened_nseqR :=
  ...
  | [ H1 : merge ?X1 ?X2 ?X3, H2 : struct_equiv_str ?X1 ?X2 |- _ ] =>
    pose proof (merge_weakened_nseqR H2 H1)
  end.
...
Lemma merge_app_struct_equiv_strR_explicit : forall {V : Set} G H GHs Γ Δ d,
  @merge V G (H ++ [(Γ,Δ,d)]) GHs ->
...
(* 
Update weakened_nseq_solve (from weakenedT) with extra end clauses. 
Not sure if it's backward compatible. 
TODO: check if all weakened_nseq_solve can be replaced by
weakened_nseq_solve2
*)

Ltac weakened_nseq_solve_nseq2 :=
  repeat match goal with
         ...
         | [ |- weakened_nseq (?l1 ++ [?a1]) (?l2 ++ [?a2]) ] =>
           eapply weakened_nseq_app
         end.

Ltac weakened_seq_solve2 :=
  repeat match goal with
         ...
         | [ H : merge ?X1 ?X2 ?X3, H2 : struct_equiv_str ?X1 ?X2 |- weakened_nseq ?X1 ?X3 ] => eapply merge_weakened_nseqL; [ eapply H2 | eapply H]
         | [ H : merge ?X1 ?X2 ?X3, H2 : struct_equiv_str ?X1 ?X2 |- weakened_nseq ?X2 ?X3 ] => eapply merge_weakened_nseqR; [ eapply H2 | eapply H]
         | [ |- weakened_seq _ _ ] => eapply weak_seq_baseLR; econstructor
         end.

Ltac weakened_nseq_solve2 :=
  weakened_nseq_solve_nseq2; 
  weakened_seq_solve2;
  try (eapply weak_seq_baseLR; econstructor);
  ...
  repeat
    (match goal with
     | [H : merge ?G1 ?G1 ?G2 |- _ ] =>
       ...
       | _ => pose proof (merge_contracted_nseq H)
       end
     | [H1 : merge ?G1 ?G2 ?G3 |- _ ] =>
       lazymatch goal with
