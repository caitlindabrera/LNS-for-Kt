...
Lemma Lemma_Sixteen_SR_bb_bac_WBox2Rs : forall n m 
  ...
  (D1 : derrec (LNSKt_rules (V:=V)) (fun _ : (@LNS V) => False)
         (G ++ [(Γ, Δ1 ++ BBox A :: Δ2, bac)]))
  ctxt AA L1 L2 L3
  (Heqconcl : nslclext ctxt [(L1, L2 ++ WBox AA :: L3, fwd)] =
             H ++ (Σ1 ++ BBox A :: Σ2, Π, bac) :: I)
  (D3 : derrec (LNSKt_rules (V:=V)) (fun _ : (@LNS V) => False)
  ...
  (Hme : merge G H GH)
  (Hsize : fsize A <= n),
  derrec (LNSKt_rules (V:=V)) (fun _ : (@LNS V) => False)
...
(* ---------------------------- *)
(* Lemma_Sixteen_SR_bb_bac_ImpR *)
(* ---------------------------- *)

Lemma Lemma_Sixteen_SR_bb_bac_ImpR : forall n m
  (IH : forall y : nat * nat, y << (S n, m) -> SR_wb y * SR_bb y * SR_p y * SL y)
...
(* ---------------------------- *)
(* Lemma_Sixteen_SR_bb_bac_ImpL *)
(* ---------------------------- *)

Lemma Lemma_Sixteen_SR_bb_bac_ImpL : forall n m
  (IH : forall y : nat * nat, y << (S n, m) -> SR_wb y * SR_bb y * SR_p y * SL y)
  V G Γ Δ1 Δ2 H Σ1 Σ2 Π I GH A d2
  (D1 : derrec (LNSKt_rules (V:=V)) (fun _ : (@LNS V) => False)
         (G ++ [(Γ, Δ1 ++ BBox A :: Δ2, d2)]))
...
Proof.
  ...
  destruct rl2' as [p | AA BB | AA BB | ].
    ...
    (* ImpR *)
    simpl in *. subst. 
    eapply Lemma_Sixteen_SR_bb_bac_ImpR; eassumption. 

    (* ImpL *) simpl in *. subst.
    eapply Lemma_Sixteen_SR_bb_bac_ImpL; eassumption.
