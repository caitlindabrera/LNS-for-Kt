...
Require Export LNSKt_calculus.
Require Export require_structural.
Require Export principal.
Require Export ind_lex.
Require Export Lia.
...
Definition SR_wb_fwd_pre (n m : nat) := forall (V : Set)
  ...
  (D1 : pf_LNSKt (G ++ [(Γ, Δ1 ++ [WBox A] ++ Δ2,fwd)]))
  (D2 : pf_LNSKt (H ++ [(Σ1 ++ [WBox A] ++ Σ2, Π, fwd)] ++ I))
  (D3 : pf_LNSKt
...
Definition SR_wb_bac_pre (n m : nat) := forall (V : Set) 
    G Γ Δ1 Δ2 H Σ1 Σ2 Π I GH (A : PropF V)
  (D1 : pf_LNSKt (G ++ [(Γ, Δ1 ++ [WBox A] ++ Δ2,bac)]))
  (D2 : pf_LNSKt (H ++ [(Σ1 ++ [WBox A] ++ Σ2, Π, bac)] ++ I))
  (D3 : pf_LNSKt
...
Definition SR_wb_pre (n m : nat) := forall (V : Set)
  ...
  (D1 : pf_LNSKt (G ++ [(Γ, Δ1 ++ [WBox A] ++ Δ2,d)]))
  (D2 : pf_LNSKt (H ++ [(Σ1 ++ [WBox A] ++ Σ2, Π, d)] ++ I))
  (D3 : pf_LNSKt
...
Definition SR_bb_fwd_pre (n m : nat) := forall (V : Set) 
    G Γ Δ1 Δ2 H Σ1 Σ2 Π I GH (A : PropF V)
  (D1 : pf_LNSKt (G ++ [(Γ, Δ1 ++ [BBox A] ++ Δ2,fwd)]))
  (D2 : pf_LNSKt (H ++ [(Σ1 ++ [BBox A] ++ Σ2, Π, fwd)] ++ I))
  (D3 : pf_LNSKt
                               (GH ++ [(Γ ++ Σ1 ++ Σ2, Δ1 ++ Δ2 ++ Π, fwd)] ++
                           [([],[A],bac)] )),
...
Definition SR_bb_bac_pre (n m : nat) := forall (V : Set)
    G Γ Δ1 Δ2 H Σ1 Σ2 Π I GH (A : PropF V)
  (D1 : pf_LNSKt (G ++ [(Γ, Δ1 ++ [BBox A] ++ Δ2,bac)]))
  (D2 : pf_LNSKt (H ++ [(Σ1 ++ [BBox A] ++ Σ2, Π, bac)] ++ I))
  (D3 : pf_LNSKt
                       (GH ++ [(Γ ++ Σ1 ++ Σ2, Δ1 ++ Δ2 ++ Π, bac)] ++
                           [([],[A],bac)] )),
        principal_BBR D1 (BBox A) Γ Δ1 Δ2 ->
...
Definition SR_bb_pre (n m : nat) := forall (V : Set)
    G Γ Δ1 Δ2 H Σ1 Σ2 Π I GH (A : PropF V) d
  (D1 : pf_LNSKt (G ++ [(Γ, Δ1 ++ [BBox A] ++ Δ2,d)]))
  (D2 : pf_LNSKt (H ++ [(Σ1 ++ [BBox A] ++ Σ2, Π, d)] ++ I))
  (D3 : pf_LNSKt
                       (GH ++ [(Γ ++ Σ1 ++ Σ2, Δ1 ++ Δ2 ++ Π, d)] ++
                           [([],[A],bac)] )),
        principal_BBR D1 (BBox A) Γ Δ1 Δ2 ->
...
Definition SR_p_pre (n m : nat) := forall (V : Set)
    G Γ Δ1 Δ2 H Σ1 Σ2 Π I GH (A : PropF V) d 
  (D1 : pf_LNSKt (G ++ [(Γ, Δ1 ++ [A] ++ Δ2,d)]))
  (D2 : pf_LNSKt (H ++ [(Σ1 ++ [A] ++ Σ2, Π, d)] ++ I)),
...
Definition SL_pre (n m : nat) := forall (V : Set) 
    G Γ Δ1 Δ2 H Σ1 Σ2 Π I GH (A : PropF V) d 
  (D1 : pf_LNSKt (G ++ [(Γ, Δ1 ++ [A] ++ Δ2,d)] ++ I))
  (D2 : pf_LNSKt (H ++ [(Σ1 ++ [A] ++ Σ2, Π, d)])),
        ((dp D1) + (dp D2))%nat <= m ->
        fsize A <= n ->
        struct_equiv_str G H ->
        merge G H GH ->
