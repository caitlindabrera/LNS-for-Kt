...
(* ---------------------- *)
(* Lemma_Sixteen_SR_wb_Id *)
(* ---------------------- *)

Lemma Lemma_Sixteen_SR_wb_Id : forall {V : Set} GH H I ctxt d d2 Γ Σ1 Σ2 Δ1 Δ2 Π Φ1 Φ2 Ψ1 Ψ2 p A,
    (nslcext ctxt d (Φ1 ++ Var p :: Φ2, Ψ1 ++ Var p :: Ψ2) =
     H ++ (Σ1 ++ [WBox A] ++ Σ2, Π, d2) :: I) ->
    @pf_LNSKt V (GH ++ (Γ ++ Σ1 ++ Σ2, Δ1 ++ Δ2 ++ Π, d2) :: I).
Proof.
...
(* ------------------------ *)
(* Lemma_Sixteen_SR_wb_BotL *)
(* ------------------------ *)

Lemma Lemma_Sixteen_SR_wb_BotL : forall {V : Set} GH H I ctxt d d2 d3 Γ Σ1 Σ2 Δ1 Δ2 Π Φ1 Φ2 Ψ1 Ψ2 A,
...
Ltac find_princ_weak_in_snd_last_comp ns ns2 :=
  ...
  | [(?Γ,?Δ,?d)] ++ [?s] =>
    match ns2 with
    | [(?Γ2,?Δ2,?d2)] ++ [?s2] => find_princ_weak_in_seq Γ Γ2
    end
  | ?GH ++ ?tl =>
    match ns2 with
...
Lemma nslclrule_b2lrules2 : forall V ps concl L L' s1 s2,
    L = L' ->
    concl = ((L ++ [s1]) ++ [s2]) ->
    b2lrules ps ([s1;s2]) ->
...
Ltac SR_wb_fwd_WBox2Ls_snd_last_comp D2 D3 AA A HSR Hdp' :=
      unf_pfall; app_eq_app_dest3; try contradiction;
     ( eapply derI;
         [
         eapply b2l;
         econstructor;
...
(IH : forall y : nat * nat, y << (S n, m) -> SR_wb y * SR_bb y * SR_p y * SL y)
  ...
  (D2s : dersrec (LNSKt_rules (V:=V)) (fun _ : (@LNS V) => False)
...
Lemma Lemma_Sixteen_SR_wb_fwd_ImpR : forall n m
...
Proof.
  ...
  (destruct (list_nil_or_tail_singleton I); sD; subst;
    inv_app_hd_tl_full;              
    [app_eq_app_dest3; try contradiction; try discriminate | ]);
  solve_case_F_gen_draft2 D1 D2 D2' D3 HSR Hdp' (@ImpR V) fill_tac_ImpR.
     Unshelve. all : (subst ; solve_eqs).
Qed.
...
Proof.
  ...
  +{ tac_cons_singleton_hyp Heqconcl.
     unfold LNS in *; unfold seq in *; app_eq_app_dest3; try contradiction.
     ...
     ++{ (* WBox A in same component as principle WBox but to its left. *)
       ...
       Unshelve. all : (subst; solve_eqs).
        ++{ (* WBox A is princ WBox. *)
         ...
         simpl in D7.
         list_assoc_r_single. rewrite cons_singleton.
...
(* ---------------------------- *)
(* Lemma_Sixteen_SR_wb_fwd_ImpL *)
(* ---------------------------- *)

Lemma Lemma_Sixteen_SR_wb_fwd_ImpL : forall n m
  (IH : forall y : nat * nat, y << (S n, m) -> SR_wb y * SR_bb y * SR_p y * SL y)
  V G Γ Δ1 Δ2 H Σ1 Σ2 Π I GH A d2
  (D1 : pf_LNSKt
         (G ++ [(Γ, Δ1 ++ WBox A :: Δ2, d2)]))
...
Proof.
  ...
  destruct rl2' as [p | AA BB | AA BB | ].
    ...
    clear D1 IH.
    eapply Lemma_Sixteen_SR_wb_Id. eassumption.
 
    (* ImpR *)
    simpl in *. subst. 
    eapply Lemma_Sixteen_SR_wb_fwd_ImpR; eassumption. 

    (* ImpL *) simpl in *. subst.
    eapply Lemma_Sixteen_SR_wb_fwd_ImpL; eassumption.

    (* Bot  *) 
    simpl in *. subst.
    clear Hsize D3 Hme Hdp D2s Hprinc.
    clear D1 IH.
    eapply Lemma_Sixteen_SR_wb_BotL. eassumption.
Qed.
