...
middle = "\\begin{figure}[h] \\[ " ++ (convert_der_to_string (output)) ++ "\\] \\end{figure}"
...
convert_der_to_string d =
  case d of {
    DdT.Coq_dpI x prems -> "AxiomC{ " ++ (convert_lns_to_string x) ++ " }";
    DdT.Coq_derI l x rl ds -> "\\UnaryInfC{ " ++ (convert_lns_to_string x) ++ " } \\RightLabel{ " ++ (convert_rule_to_string rl) ++ " }" ++ (convert_ders_to_string ds)}
...
convert_ders_to_string ds = 
  case ds of {
    DdT.Coq_dlNil -> "\\AxiomC{~} ";
    DdT.Coq_dlCons x l d ds -> (convert_der_to_string d) ++ " " ++ (convert_ders_to_string ds)}
{-
    DdT.Coq_dlCons x l d (DdT.Coq_dlNil) -> "\\UnaryInfC{ " ++ (convert_lns_to_string x) ++ " }" ++ (convert_der_to_string d)
    DdT.Coq_dlCons x l d (DdT.Coq_dlCons y l' d') -> "\\BinaryInfC{ " ++ (convert_lns_to_string x) ++ " }" ++ (convert_der_to_string d) ++ (convert_ders_to_string d')

    DdT.Coq_dlCons x l d ds -> "dlCons " ++ (convert_lns_to_string x) ++ (convert_der_to_string d) ++ " " ++ (convert_ders_to_string ds)}
-}


convert_rule_to_string :: (Lntkt_exchT.LNSKt_rules Vars) -> String
convert_rule_to_string rl =
  case rl of {
    Lntkt_exchT.Coq_b2r _ _ _ -> "box2r";
    Lntkt_exchT.Coq_b1r _ _ _ -> "box1r";
    Lntkt_exchT.Coq_b2l _ _ _ -> "box2l";
    Lntkt_exchT.Coq_b1l _ _ _ -> "box1l";
    Lntkt_exchT.Coq_nEW _ _ _ -> "$\\EW$";
    Lntkt_exchT.Coq_prop _ _ _ -> "prop"}
