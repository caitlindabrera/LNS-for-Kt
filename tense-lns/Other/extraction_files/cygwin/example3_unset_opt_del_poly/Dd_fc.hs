...
dersrecD_forall_in_dersrec cs ds =
  DdT.dersrec_rect (\_ hin ->
    (case hin of {
      ...
      GenT.InT_cons _ _ x0 -> (\_ -> Logic.coq_False_rect x0)}) __)
    (\seq seqs d ds0 iHds c hin ->
    (case hin of {
      GenT.InT_eq' b l -> (\_ ->
       Logic.eq_rect_r seq (\_ ->
         Logic.eq_rect_r seqs (\_ ->
           Logic.eq_rect_r c (\d0 _ _ -> Specif.Coq_existT d0 (Coq_in_dersrec_hd seqs
             ds0)) seq d hin __) l) b __ __);
      GenT.InT_cons b l x0 -> (\_ ->
       Logic.eq_rect_r seq (\_ ->
         Logic.eq_rect_r seqs (\x1 ->
           let {x2 = iHds c x1} in
           case x2 of {
            Specif.Coq_existT d2 hin2 -> Specif.Coq_existT d2 (Coq_in_dersrec_tl seq d
             seqs ds0 hin2)}) l) b __ x0)}) __) cs ds
          ...
          Datatypes.Coq_nil -> (\_ d5 d2'1 _ _ ->
           let {
            s = dersrec_derrec_height
                  (dersrec_height (Datatypes.Coq_cons x Datatypes.Coq_nil) d5) x d5}
           in
           case s of {
            Specif.Coq_existT x0 _ ->
             Logic.eq_rect_r (DdT.Coq_dlCons seq (Datatypes.Coq_cons x Datatypes.Coq_nil)
               d d5) (\_ ->
               Logic.eq_rect
                 (dersrec_height (Datatypes.Coq_cons seq (Datatypes.Coq_cons x
                   Datatypes.Coq_nil)) (DdT.Coq_dlCons seq (Datatypes.Coq_cons x
                   Datatypes.Coq_nil) d d5))
                 (Logic.eq_rect_r g1 (\_ d0 ->
                   Logic.eq_rect_r g2 (\_ _ x1 _ -> Specif.Coq_existT d0
                     (Specif.Coq_existT x1 __)) x __ d5 x0 __) seq __ d) n) d2'1 __});
          Datatypes.Coq_cons _ _ -> (\_ _ _ _ _ -> Logic.coq_False_rect)}) __ d4 d2'0 __
          __)}) __ d3 d2' __ __)}) __ d2 __ __
